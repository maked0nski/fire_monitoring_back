...
export class FuelCardController {
  ...
  @CustomOkResponse({
    ...
    exampleData: SWAGGER_EXAMPLE_ARRAY_FUEL_CARD_LIST,
  })
  ...
  @CustomOkResponse({
    status: HttpStatus.OK,
    exampleData: SWAGGER_EXAMPLE_ONE_FUEL_CARD,
  })
  ...
  @Post()
  createFuelCard(
    @Body() createFuelCardDto: CreateFuelCardDto
  ...
  updateFuelCardById(
    @Param("id") id: string,
    @Body() updateFuelCardDto: UpdateFuelCardDto
  ): Promise<CreateFuelCardDto> {
    return this.fuelCardService.updateFuelCardById(
      Number(id),
      updateFuelCardDto
    );
  }
