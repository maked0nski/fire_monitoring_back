...
export class FuelCardService {
  ...
  createFuelCard(data: CreateFuelCardDto): Promise<CreateFuelCardDto> {
    return Promise.resolve(this.prismaService.fuel_card.create({ data })).catch(
      (error) => {
        if (
  ...
  getAllFuelCards(): Promise<CreateFuelCardDto[]> {
    return Promise.resolve(this.prismaService.fuel_card.findMany()).catch(
      (error) => {
        throw new HttpException(error.message, HttpStatus.BAD_REQUEST);
      }
    );
  ...
  getFuelCardById(id: number): Promise<CreateFuelCardDto> {
    return Promise.resolve(
      this.prismaService.fuel_card.findUniqueOrThrow({
        where: { id },
      })
    ).catch((error) => {
  ...
  updateFuelCardById(
    id: number,
    data: UpdateFuelCardDto
  ): Promise<CreateFuelCardDto> {
    return Promise.resolve(
      this.prismaService.fuel_card.update({
        where: { id },
        data,
      })
    ).catch((error) => {
  ...
  deleteFuelCardById(id: number): Promise<CreateFuelCardDto> {
    return Promise.resolve(
      this.prismaService.fuel_card.delete({
        where: { id },
      })
    ).catch((error) => {
      if (error instanceof PrismaClientKnownRequestError) {
