...
export class PositionService {
  ...
  create(data: PositionDto): Promise<PositionDto> {
    return Promise.resolve(this.prismaService.position.create({ data })).catch(
      (error) => {
        if (
  ...
  getAll(): Promise<PositionDto[]> {
    return Promise.resolve(this.prismaService.position.findMany()).catch(
      (error) => {
        throw new HttpException(error.message, HttpStatus.BAD_REQUEST);
      }
    );
  ...
  getById(id: number): Promise<PositionDto> {
    return Promise.resolve(
      this.prismaService.position.findUniqueOrThrow({
        where: {
          id,
        },
      })
  ...
  delete(id: number): Promise<PositionDto> {
    return Promise.resolve(
      this.prismaService.position.delete({
        where: { id },
      })
    ).catch((error) => {
      if (
