...
export class ObservationService {
  ...
  create(dto: CreateObservationDto): Promise<CreateObservationDto> {
    return Promise.resolve(
      this.prismaService.observation.create({
        data: dto,
        select: {
          id: true,
          number: true,
  ...
  findById(id: number): Promise<CreateObservationDto> {
    return Promise.resolve(
      this.prismaService.observation.findFirstOrThrow({
        where: { id },
        select: {
          id: true,
          number: true,
  ...
  update(id: number, dto: UpdateObservationDto): Promise<CreateObservationDto> {
    return Promise.resolve(
      this.prismaService.observation.update({
        where: { id },
        data: dto,
        select: {
          id: true,
  ...
  delete(id: number): Promise<any> {
    return Promise.resolve(
      this.prismaService.observation.delete({
        where: { id },
      })
    ).catch((error) => {
      if (
