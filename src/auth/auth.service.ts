...
export class AuthService {
  ...
  async signUp(userDto: AuthUserDto): Promise<boolean> {
    ...
    if (!foundUser) {
      // якщо нема користувача з даною поштою
  ...
  async verifyToken(token): Promise<any> {
    ...
    try {
      ...
      const userFromBD = await this.prismaService.user.findUnique({
        where: {
          id: data?.id,
        },
      });
      if (userFromBD && moment().isBefore(userFromBD.verificationCodeAt)) {
        //якщо протермінований код заново вислати
        return userFromBD;
      }
    } catch (error) {
      return false;
